...
# Import ICT analysis components
from .ict_analyzer import (
    OrderBlockDetector, FairValueGapAnalyzer, MarketStructureAnalyzer,
    TimeFrameManager, PatternValidationEngine, PatternType,
    OrderBlock, FairValueGap, MarketStructure, PatternValidationResult
)
...
class TechnicalAnalyzer:
    ...
    def analyze_comprehensive(self,
        ...
        try:
            logger.info(f"Starting comprehensive ICT analysis for {len(data)} bars on {timeframe}")
            ...
            if self.performance_monitoring:
                self._record_performance_metrics(analysis_result, results['component_times'], len(data))
    ...
    def analyze_single_timeframe(self,
                                data: pd.DataFrame,
                                timeframe: str,
        ...
        try:
            logger.info(f"Single timeframe analysis: {timeframe} with components: {components}")

            # Order Block Detection
            if 'order_blocks' in components:
                swing_points = self.order_block_detector.find_swing_points(data)
                order_blocks = self.order_block_detector.identify_order_blocks(data, swing_points)
            ...
            if 'structure' in components:
                swing_points = results.get('swing_points', self.order_block_detector.find_swing_points(data))
                market_structures = self.market_structure_analyzer.analyze_structure(data, swing_points)
                results['market_structures'] = market_structures
                logger.debug(f"Found {len(market_structures)} market structure patterns")
    ...
    def get_real_time_signals(self,
                             data: pd.DataFrame,
                             lookback_bars: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if swing_points:
                latest_ob = self.order_block_detector.identify_order_blocks(recent_data, swing_points)
                if latest_ob:
                    ob_signal = self._generate_order_block_signal(latest_ob[-1], recent_data)
                    if ob_signal:
            ...
            structures = self.market_structure_analyzer.detect_structural_breaks(recent_data)
            for structure_type, structure_list in structures.items():
                if structure_list:
                    structure_signal = self._generate_structure_signal(structure_list[-1], recent_data)
                    if structure_signal:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_analysis_time = np.mean([p.total_analysis_time for p in self.performance_history])
            avg_memory_usage = np.mean([p.memory_usage_mb for p in self.performance_history])
            avg_patterns_per_second = np.mean([p.patterns_per_second for p in self.performance_history])
    ...
    def _run_parallel_analysis(self,
        ...
        try:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                # Order Block detection
                futures['order_blocks'] = executor.submit(self._analyze_order_blocks, data)
                ...
                futures['structure'] = executor.submit(self._analyze_market_structure, data)
                ...
                for component, future in futures.items():
                    try:
                        component_result = future.result(timeout=30)  # 30 second timeout
        ...
        try:
            ...
            if self.enable_validation and order_blocks:
                for ob in order_blocks[-5:]:  # Validate last 5 patterns
                    validation = self.validation_engine.validate_pattern(ob, data, PatternType.ORDER_BLOCK)
        ...
        try:
            ...
            if self.enable_validation and fair_value_gaps:
                for fvg in fair_value_gaps[-3:]:  # Validate last 3 patterns
                    validation = self.validation_engine.validate_pattern(fvg, data, PatternType.FAIR_VALUE_GAP)
                    validations.append(validation)
    ...
    def _analyze_market_structure(self, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            structural_breaks = self.market_structure_analyzer.detect_structural_breaks(data)

            # Pattern validation if enabled
            validations = []
            if self.enable_validation and market_structures:
                for ms in market_structures[-3:]:  # Validate last 3 patterns
                    pattern_type = PatternType.BREAK_OF_STRUCTURE if ms.pattern_type == "BOS" else PatternType.CHANGE_OF_CHARACTER
                    validation = self.validation_engine.validate_pattern(ms, data, pattern_type)
                    validations.append(validation)
            ...
            return {'market_structures': [], 'structural_breaks': {}, 'structure_validations': []}
    ...
    def _analyze_confluence(self, results: Dict, data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if indicators and any(ict_patterns.values()):
                fusion_analysis = self.pattern_indicator_fusion.fusion_analysis(data, ict_patterns)
    ...
    def _generate_analysis_summary(self, results: Dict, duration: float) -> Dict[str, Any]:
    ...
    def _record_performance_metrics(self, result: AnalysisResult, component_times: Dict, data_size: int) -> None:
        ...
        try:
            ...
            patterns_per_second = patterns_found / result.analysis_duration if result.analysis_duration > 0 else 0
    ...
    def _optimize_memory(self) -> None:
        ...
        try:
            if self.memory_optimization:
                ...
                if hasattr(self, 'validation_engine') and hasattr(self.validation_engine, 'reliability_cache'):
        ...
        try:
            if self.memory_optimization:
                ...
                if current_memory > self._memory_threshold_mb:
                    logger.info(f"Memory usage ({current_memory:.1f}MB) exceeds threshold, performing cleanup")
                    ...
                    if hasattr(self.timeframe_manager, 'timeframe_data'):
                        for tf_name, tf_data in self.timeframe_manager.timeframe_data.items():
                            if hasattr(tf_data, 'data') and len(tf_data.data) > 1000:
                                tf_data.data = tf_data.data.tail(500).copy()

                        logger.debug("Cleaned up timeframe data for memory optimization")
    ...
    def _generate_order_block_signal(self, order_block: OrderBlock, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            atr = data['close'].rolling(14).apply(lambda x: np.mean(np.abs(x.diff()))).iloc[-1]
    ...
    def _generate_fvg_signal(self, fvg: FairValueGap, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    def _generate_structure_signal(self, structure: MarketStructure, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    def _filter_and_rank_signals(self, signals: List[Dict[str, Any]], data: pd.DataFrame) -> List[Dict[str, Any]]:
