...
class IntegratedSignalSystem:
    ...
    def __init__(
        ...
        # Note: SignalGenerator requires dependencies that would need to be injected
        # For now, initialize to None and set up when dependencies are available
    ...
    async def process_market_data(
        ...
        try:
            ...
            if not confluence_result.is_valid:
                logger.debug(
                    f"Signal rejected by confluence validation: {confluence_result.rejection_reasons}"
                )
                return published_signals
            ...
            logger.debug(
                f"Signal passed confluence validation with score: {confluence_result.total_score}"
            )
            ...
            raw_signal["strength"] = {
                ...
                "scores": {
                    cat.value: score
                    for cat, score in strength_result.category_scores.items()
                },
                "risk_reward_ratio": strength_result.risk_reward_ratio,
            }

            logger.debug(
                f"Signal strength calculated: {strength_result.category.value} ({strength_result.total_score})"
            )
            ...
            if not bias_result.is_valid:
                logger.debug(
                    f"Signal filtered out by bias filter: {bias_result.rejection_reasons}"
                )
                return published_signals
            ...
            raw_signal["bias_filter"] = {
                "session": (
                    bias_result.session_info.session_type.value
                    if bias_result.session_info
                    else None
                ),
                "bias_direction": (
                    bias_result.bias_direction.value
                    if bias_result.bias_direction
                    else None
                ),
            ...
            logger.debug(
                f"Signal passed bias filtering for {bias_result.session_info.session_type.value if bias_result.session_info else 'unknown'} session"
            )
            ...
            publish_success = await self.event_publisher.publish_signal(
                validated_signal,
                priority=self._determine_signal_priority(validated_signal),
            )
    ...
    def mark_signal_executed(
        self, signal_id: str, execution_data: Dict[str, Any]
    ) -> bool:
        """
    ...
    def cancel_signal(
        self, signal_id: str, reason: str = "manual_cancellation"
    ) -> bool:
    ...
    def _calculate_position_size(
        self, signal: Dict[str, Any], strength_result
    ) -> Dict[str, Any]:
    ...
    def _setup_validity_callbacks(self):
        ...
        def on_signal_expired(validity_info):
            logger.info(
                f"Signal {validity_info.signal_id} expired: {validity_info.validity_reason.value if validity_info.validity_reason else 'unknown'}"
            )
        ...
        self.validity_manager.register_state_callback(
            SignalState.EXECUTED, on_signal_executed
        )
        self.validity_manager.register_state_callback(
            SignalState.ARCHIVED, on_signal_archived
        )
...
async def example_usage():
    ...
    try:
        ...
        signals = await signal_system.process_market_data(
            market_data, metadata={"symbol": "EURUSD", "session": "london"}
        )
