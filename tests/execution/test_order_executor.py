...
from src.trading_bot.core.events import OrderEvent, EventType, OrderSide, OrderType as OrderTypeEnum, OrderStatus
...
async def test_execute_order_success(order_executor, mock_binance_client, sample_order_request):
    ...
    mock_binance_client.futures_create_order.return_value = mock_response
...
async def test_execute_order_failure(order_executor, mock_binance_client, sample_order_request):
    """주문 실행 실패 테스트"""
    # Given
    mock_binance_client.futures_create_order.side_effect = Exception("Network error")
...
@pytest.mark.asyncio
async def test_cancel_order(order_executor, mock_binance_client, sample_order_request):
    """주문 취소 테스트"""
    # Given - First execute an order to populate active_orders
    ...
    mock_binance_client.futures_create_order.return_value = mock_create_response
    await order_executor.execute_order(sample_order_request)
    ...
    # When
    result = await order_executor.cancel_order('test_order_001')
...
async def test_order_executor_start_stop(order_executor):
    ...
    assert order_executor.is_running()
    
    await order_executor.stop()
    assert not order_executor.is_running()
